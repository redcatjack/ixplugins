...
SWEP.StunModes = {low = 5, medium = 10, high = 15}
SWEP.CurrentStunMode = "low"
...
function SWEP:Initialize()
    ...
    self.NextStunModeSwitch = CurTime()  -- Initialize NextStunModeSwitch
end
...
function SWEP:PrimaryAttack()
        ...
        if (SERVER) then
            ...
            if (state) then
                self.Owner:EmitSound("Weapon_StunStick.Activate")
                            ix.chat.Send(self.Owner, "me", "switches their stunstick to " .. self.CurrentStunMode .. " power mode.")
    ...
    if (self:GetActivated()) then
        damage = self.StunModes[self.CurrentStunMode]
    end
...
function SWEP:Reload()
    if (CurTime() < self.NextStunModeSwitch) then
        return
    end

    if (self.Owner:KeyDown(IN_WALK) and self:GetActivated()) then  -- Ensure stun mode is active
        if (SERVER) then
            local nextMode = self:GetNextStunMode()
            self:SetStunMode(nextMode)
            self.Owner:EmitSound("Weapon_StunStick.Activate")
            ix.chat.Send(self.Owner, "me", "switches their stunstick to " .. nextMode .. " power mode.")
            self.NextStunModeSwitch = CurTime() + 1  -- Adding a cooldown of 1 second
        end
    end
end

function SWEP:GetNextStunMode()
    if (self.CurrentStunMode == "low") then
        return "medium"
    elseif (self.CurrentStunMode == "medium") then
        return "high"
    else
        return "low"
    end
...
function SWEP:SetStunMode(mode)
    self.CurrentStunMode = mode
end
