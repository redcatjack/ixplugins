...
ENT.PrintName = "Thin Single Door - DOWN"
...
function ENT:Initialize()
    self:SetModel("models/props/redcat/cmbdoors/cmb_door_single_thin.mdl")  -- Set your desired model
    self:PhysicsInit(SOLID_VPHYSICS)
    self:SetMoveType(MOVETYPE_VPHYSICS)  -- Prevent physics interactions
    self:SetSolid(SOLID_VPHYSICS)
    self:SetAngles(Angle(180, 0, 0))  -- Rotate the door 180 degrees to make it upside down
...
function ENT:MoveDoor(down)
    local targetPos
    local moveAmount = self:OBBMaxs().z * 1.99  -- Move 1.99 times the model's height
    if down then
    ...
    local duration = 5  -- Duration of the movement in seconds
...
-- Register the entity
scripted_ents.Register(ENT, "single_thin_door_down")
